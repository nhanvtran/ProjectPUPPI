...
puppiCleanContainer::puppiCleanContainer(std::vector<fastjet::PseudoJet> inParticles, std::vector<int> isPU, std::vector<int> isCh,bool iDiscretize){
  ...
  _vals.resize(0); 
   for (unsigned int i = 0; i < inParticles.size(); i++){
    ...
    if (inParticles[i].user_index() < 4) _genParticles.push_back( inParticles[i] );            
    //if (fabs(inParticles[i].user_index()) % 4 < 2 && iDiscretize)  continue;
    _pfParticles.push_back(inParticles[i]);
    if (inParticles[i].user_index() % 4 != 3) { 
      _pfchsParticles.push_back( inParticles[i] );
    }
    if (inParticles[i].user_index() % 4 == 2) { 
...
void puppiCleanContainer::discretize(std::vector<fastjet::PseudoJet> &discreteParticles,std::vector<fastjet::PseudoJet> &iParticles,bool iPtCut) {
  ...
  for (unsigned int i0 = 0; i0 < iParticles.size(); i0++){
    ...
    if (lGrid  && iParticles[i0].user_index() % 4 == 0) h2d     ->SetBinContent( curBin, h2d     ->GetBinContent(curBin) + iParticles[i0].e() );        
    if (lGrid  && iParticles[i0].user_index() % 4 == 1) h2dPU   ->SetBinContent( curBin, h2dPU   ->GetBinContent(curBin) + iParticles[i0].e() );        
    if (!lGrid && iParticles[i0].user_index() % 4 == 0) lJets  [curBin] += iParticles[i0];
    if (!lGrid && iParticles[i0].user_index() % 4 == 1) lJetsPU[curBin] += iParticles[i0];
  }
  //For now sumit up
  if(!lGrid) { 
...
std::vector<fastjet::PseudoJet> puppiCleanContainer::puppiEvent     (int iOpt,double iQuant) { 
  ...
  for(int i0 = 0; i0 < lNEvents; i0++) {
    ...
    if(_pfParticles[i0].user_index() % 4 < 2 && pWeight*_pfParticles[i0].pt() < 1.0) continue;
